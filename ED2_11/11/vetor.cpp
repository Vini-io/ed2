...
vetor::vetor(int TAM) : array(nullptr), arraySelection(nullptr), arrayInsertion(nullptr), arrayBubble(nullptr), arrayMerge(nullptr), arrayHeap(nullptr), arrayQuick(nullptr), tam(0), excSelection(0), excInsertion(0), excBubble(0), execMerge(0), execHeap(0), execQuick(0){
    ...
    try{
        ...
        arrayHeap = new int[TAM];
        arrayQuick = new int[TAM];
...
void vetor::insertArray(){
    ...
    for(int i = 0; i < tam; i++){
       arrayQuick[i] = arrayHeap[i] = arrayMerge[i] = arrayBubble[i] = arrayInsertion[i] = arraySelection[i] = array[i] = distr(gen);
...
int vetor::getElementArray(int i, int arr){
    ...
    else if(arr == 6) return arrayHeap[i];
    else if(arr == 7) return arrayQuick[i];
}
...
void vetor::heapif(int n, int i)
{
    ...
    if (filho_esquerda < n && arrayHeap[filho_esquerda] > arrayHeap[maior]){
         execHeap++;
        maior = filho_esquerda;
    }
    if (filho_direita < n && arrayHeap[filho_direita] > arrayHeap[maior]){
         execHeap++;
        maior = filho_direita;
    }
    if (maior != i) {
        std::swap(arrayHeap[i], arrayHeap[maior]);
         execHeap++;
        heapif(n, maior);
    }
...
void vetor::heapSort() {
    ...
    for (int i = n - 1; i >= 0; i--) {
        std::swap(arrayHeap[0], arrayHeap[i]);
        heapif(i, 0);
    }
}
...
int vetor::partition(int low, int high)
{
    ...
    for (int j = low; j < high; j++) {
        if (arrayQuick[j] <= pivot) {
            i++;
            swap(&arrayQuick[i], &arrayQuick[j]);
            execQuick++;
        }
    }

    swap(&arrayQuick[i + 1], &arrayQuick[high]);
    execQuick++;
    return (i + 1);
}
