...
int numuser::accessPosArray(int value, int inicio, int fim, int meio)
{

    if(inicio == fim && value != array[meio]){
        throw QString ("Valor não encontrado");
        return -1;
    }
    ...
    if(value == array[meio]){
        ...
        //direita
        inicio = meio + 1;
        accessPosArray(value, inicio, fim, ((meio + 1) + fim) / 2);
    }else{
        //esquerda
        fim = meio - 1;
        accessPosArray(value, inicio, fim, ((meio - 1) + inicio) / 2);
    }
    /*
    for(int i = 0; i < tam; i++){
